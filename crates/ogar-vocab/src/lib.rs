...
const CODEBOOK: &[(&str, u16)] = &[
    ...
    // PDF→text plan mints (tesseract-rs `pdf-to-text-ocr-v1.md` Phase 0 D0.3;
    // the plan IS the emit-seam declaration — each row names its consumer
    // phase, honoring the mint-on-emit guard rather than pre-minting blind).
    ("textline", 0x0805),
    ("blob", 0x0806),
    ("page_layout", 0x0807),
    ("page_image", 0x0808),
...
pub mod class_ids {
    ...
    /// `textline` (`0x0805`) — a segmented text line: box + baseline, the unit
    /// the LSTM line recognizer consumes and the word/box extract references
    /// (plan Phases 1B + 3E; the line images `RecognizeLine` reads).
    pub const TEXTLINE: u16 = 0x0805;
    /// `blob` (`0x0806`) — a connected-component blob container (Tesseract
    /// `C_BLOB`/`BLOBNBOX` kind), the page-segmentation substrate (plan
    /// Phase 3B/3D). Content (outlines/pixels) stays in content stores.
    pub const BLOB: u16 = 0x0806;
    /// `page_layout` (`0x0807`) — the layout-analysis result container:
    /// blocks / columns / reading order over one page (plan Phase 3F/4A).
    pub const PAGE_LAYOUT: u16 = 0x0807;
    /// `page_image` (`0x0808`) — a rasterized page container (decoded +
    /// grey/binary normalized), the OCR input kind (plan Phase 2).
    pub const PAGE_IMAGE: u16 = 0x0808;
    /// `ocr_renderer` (`0x0809`) — an output-rendering KIND (txt / tsv /
    /// hOCR / searchable-PDF), one slot; the concrete format is the
...
/// Textline (`0x0805`) — a segmented text line (box + baseline), the unit the
/// LSTM line recognizer consumes (plan Phases 1B + 3E).
#[must_use]
pub fn textline() -> Class {
    let mut c = Class::new("Textline");
    c.language = Language::Unknown;
    c.canonical_concept = Some("textline".to_string());
    let mut baseline_y = Attribute::new("baseline_y");
    baseline_y.type_name = Some("i32".to_string());
    c.attributes = vec![baseline_y];
    c.associations = vec![family_edge("belongs_to", "PageLayout")];
    c
}

/// Blob (`0x0806`) — a connected-component blob container (Tesseract
/// `C_BLOB`/`BLOBNBOX` kind), the page-segmentation substrate (plan Phase 3B/3D).
#[must_use]
pub fn blob() -> Class {
    let mut c = Class::new("Blob");
    c.language = Language::Unknown;
    c.canonical_concept = Some("blob".to_string());
    c.associations = vec![family_edge("belongs_to", "Textline")];
    c
}

/// PageLayout (`0x0807`) — the layout-analysis result container: blocks /
/// columns / reading order over one page (plan Phase 3F/4A).
#[must_use]
pub fn page_layout() -> Class {
    let mut c = Class::new("PageLayout");
    c.language = Language::Unknown;
    c.canonical_concept = Some("page_layout".to_string());
    c.associations = vec![family_edge("derived_from", "PageImage")];
    c
}

/// PageImage (`0x0808`) — a rasterized page container (decoded + grey/binary
/// normalized), the OCR input kind (plan Phase 2).
#[must_use]
pub fn page_image() -> Class {
    let mut c = Class::new("PageImage");
    c.language = Language::Unknown;
    c.canonical_concept = Some("page_image".to_string());
    let mut depth = Attribute::new("depth");
    depth.type_name = Some("u8".to_string());
    c.attributes = vec![depth];
    c
}

/// OcrRenderer (`0x0809`) — an output-rendering KIND (txt / tsv / hOCR /
/// searchable-PDF), one slot; the concrete format is the classid custom-low
/// half, mirroring the `network_layer` pattern (plan Phase 4B-D).
#[must_use]
pub fn ocr_renderer() -> Class {
    let mut c = Class::new("OcrRenderer");
    c.language = Language::Unknown;
    c.canonical_concept = Some("ocr_renderer".to_string());
