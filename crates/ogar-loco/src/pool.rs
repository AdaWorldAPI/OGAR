...
//! # Why a pool at all
//!
//! A [`Call`](crate::Call)'s immediate is one byte under
//! [`LaneShape::Pairs`](crate::LaneShape). A numeric field is a double; a
//! text field is arbitrary UTF-8. Neither fits. So the byte becomes an
//! **index**, and the only real design question is where the indexed bytes
//! live and what classid governs their reading.
//!
...
//!
//! - **Encode the literal as a run of calls** (no pool). Killed by the
//!   one-write gate, not by aesthetics: the call count would depend on the
...
//! classid, because minting constant-type concepts is an operator decision
//! with a ledger entry, and this crate does not get to assume one. The
//! arithmetic and the dedup — which is where the defects live — are testable
//! today against [`placeholder`] classids; the mint changes two constants,
//! not the logic.

use crate::{CLASSID_BYTES, CONTENT_SLOTS, PAYLOAD_BYTES_PER_SLOT, SLOT_STRIDE};
...
/// is loud rather than plausible. The mint proposal lives in the block-editor
/// plan's ledger; nothing here assumes it.
pub mod placeholder {
...
///
/// Held beside the [`FunctionBody`](crate::FunctionBody), never inside it —
/// the body's 30 lanes stay 30 call lanes, and its capacity arithmetic is
...
mod tests {
    ...
    fn the_same_value_interns_once_and_two_values_interns_twice() {
        // Two-sided. The first half alone passes a pool that returns index 1
        // for everything; the second alone passes a pool that never dedups.
        // Both are needed.
        let mut pool = ConstantPool::new();
