...
//!
//! The V3 substrate reads every 12-byte payload as **6 × (u8:u8)** — six
//! two-byte rails, either semantic pairs or **indexed** into a codebook. Block
//! content takes the indexed reading, and the pair is
//! **[`Call`] = `(function : value)`**:
//!
//! ```text
...
//! `0x40`; a user-defined block is another index in the same `<256` codebook;
//! invoking either is the same two bytes. What used to look like a palette of
//! operations is simply the low range of the function codebook — see
//! [`FnIndex`].
//!
...
//! destroy. The shape is uniform within one body because it comes from that
//! body's classid.
//!
//! ## Nesting is by reference, not by delimiter
//!
//! A function index can name *another function*, so `IF` calls a body living in
//! its own node. There is no `END` marker, no jump offset, and no need for one
//! — the same way Scratch's own SB3 format nests via block references rather
//! than implicit length. An earlier pass of this crate treated the absence of a
//! stream delimiter as a defect; under `(function : value)` the question does
//! not arise.
//!
//! ## Budgets
//!
//! **A body is capped at [`LaneShape::calls_per_function`] — 180 / 120 / 90
//! depending on the shape, always 360 bytes — and the cap is enforced**
//! ([`FunctionBody::push`] / [`FunctionBody::from_calls`]). Over-length is a
//! **split into two functions**, never a bigger row — the substrate's own rule
//! (*scale is the next cascade level, never field-widening*) applied to program
//! structure.
//!
//! The codebook is capped at **`<256` functions per scope** by the same logic:
//! one byte names any function in scope, and scopes cascade rather than widen.
//!
//! Only ONE concept id is spent on content ([`BlockConcept::Content`]) — calls
//! live in the payload, not in the classid space.
//!
//! ## Wide literals
//!
//! A value byte holds `0..=255`. A call needing more (`WAIT:1.5`, a string)
//! spends its value byte as a **constant-pool index** instead of the value —
//! same pair shape, different codebook. The pool is a named follow-up; nothing
//! in the encoding changes when it lands.
//!
...
/// primitive low range of the same `<256` codebook that user-defined functions
/// mint into (resolved through the [`SoaSplit::Inventory`] registry, which is
/// the label codebook these indices point at). A [`Call`]'s first byte is a
/// `FnIndex`; the editor's pick-from palette is a *rendering* of this codebook.
///
...
impl FnIndex {
    ...
    /// [`DEVICE_FAMILY_FLOOR`] is shared. [`FnIndex::NOP`] is not an
...
mod tests {
    ...
    fn shared_core_and_device_family_partition_the_palette() {
        ...
        assert!(!FnIndex::LT.is_device_family());
        ...
        assert!(!FnIndex::NOP.is_device_family());
    }
